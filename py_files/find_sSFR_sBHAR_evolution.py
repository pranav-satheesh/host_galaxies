...
def find_matches_bw_descendants_and_control_samples(descendant_file, pop_control):
    """Find indices where descendant file post-merger matches control sample post-merger."""
    store_matches = []
...
def process_single_chain(match_idx, descendant_file, redshifts, basePath, h, sim_type, fields):
    ...
    for i in range(descendant_chain_length):
        ...
        if sim_type == 'TNG':
            subhalos = il.groupcat.loadSubhalos(basePath, snap_i, fields=fields)
            ...
            MBH_most_massive_BH, Mdot_most_massive_BH = find_most_massive_BHAR(snap_i, subhalo_id_i, redshifts, basePath, h)
    ...
    return sSFR_descendant_chain, sBHAR_descendant_chain, time_since_merger_chain, descendant_chain_length
...
if __name__ == "__main__":
    ...
    if sim_type == 'TNG':
        pop_control = control.control_samples_TNG(pop)
        # Load TNG redshifts
    ...
    match_ids = find_matches_bw_descendants_and_control_samples(descendant_file, pop_control)
    print(f"Found {len(match_ids)} matches between descendant file and control sample")
