...
def Lbol_from_BHAR(BHAR,er=0.2):
    #BHAR is taken in Msun.yr^-1
    Lbol = 5.665 * 1e46 * (er/(1-er)) * BHAR #erg/s
    return Lbol
...
def calculate_ks_hellinger_statistics(sim_objs, sim_names):
    """
    Calculate KS statistics and Hellinger distances for z and Mstar distributions
    comparing merging vs control populations for all simulations.
    
    Parameters
    ----------
    sim_objs : dict
        Dictionary of simulation objects keyed by simulation name
    sim_names : list
        List of simulation names to process
        
    Returns
    -------
    results : dict
        Dictionary containing KS and Hellinger statistics for each simulation
        with keys: 'ks_z', 'ks_mstar', 'hd_z', 'hd_mstar', 'p_z', 'p_mstar'
    """
    ...
    for sim_name in sim_names:
        ...
        z_control = sim_obj.z_control_pop
        
        # Stellar mass distributions (log10)
        mstar_merging = np.log10(sim_obj.Mstar_merging_pop)
        mstar_control = np.log10(sim_obj.Mstar_control_pop)
        
        # KS tests
        ks_z, p_z = ks_2samp(z_merging, z_control)
        ks_mstar, p_mstar = ks_2samp(mstar_merging, mstar_control)
        
        # Hellinger distances
        hd_z = hellinger_distance(z_merging, z_control, bins=30)
        hd_mstar = hellinger_distance(mstar_merging, mstar_control, bins=30)
...
def print_ks_hellinger_summary(results):
    """Print formatted summary of KS and Hellinger statistics."""
    print(f"{'Simulation':<30} {'HD(z)':<10} {'KS(z)':<10} {'p(z)':<12} {'HD(Mstar)':<12} {'KS(Mstar)':<12} {'p(Mstar)':<12}")
    print("-" * 110)
    
    for sim_name, stats in results.items():
        print(f"{sim_name:<30} {stats['hd_z']:<10.4f} {stats['ks_z']:<10.4f} {stats['p_z']:<12.3e} {stats['hd_mstar']:<12.4f} {stats['ks_mstar']:<12.4f} {stats['p_mstar']:<12.3e}")
...
def filter_sim_object_to_multiple_mergers(sim_obj, round_decimals=8):
    """
    Return a filtered simulation object containing only duplicate merger entries.

    Duplicate entries are defined using the key:
    (z, subhalo_id, log10(Mstar))

    This is designed to capture "multiple mergers" where the same merger-pop row
    appears multiple times (often differing only in progenitor bookkeeping).

    Parameters
    ----------
    sim_obj : object
        Simulation/control object with merger arrays and (optionally)
        sim_obj.pop['merging_population'].
    round_decimals : int
        Decimal precision used for log10(Mstar) in duplicate keying.

    Returns
    -------
    sim_obj_filtered : object
        Shallow copy of sim_obj where merger-aligned arrays are filtered to only
        the duplicate-entry subset.
    """

    if not hasattr(sim_obj, 'z_merging_pop'):
        raise AttributeError("sim_obj must have attribute 'z_merging_pop'.")

    n_mergers = len(sim_obj.z_merging_pop)

    # Prefer raw merger-pop table when available so keying matches population rows.
    mp = None
    if hasattr(sim_obj, 'pop') and isinstance(sim_obj.pop, dict) and 'merging_population' in sim_obj.pop:
        mp = sim_obj.pop['merging_population']

    if mp is not None:
        mp_fields = list(mp.keys())
        z_data = np.asarray(mp['z'])

        id_field_candidates = ['subhalo_id', 'subhalo_ids', 'SubfindID', 'subfind_id', 'SubhaloID', 'prog_subhalo_id']
        id_field = next((f for f in id_field_candidates if f in mp_fields), None)
        if id_field is None:
            raise KeyError(
                f"No recognized subhalo id field found in merging_population. Candidates: {id_field_candidates}"
            )

        sid_raw = np.asarray(mp[id_field])
        if 'Mstar' in mp_fields:
            mstar_data = np.asarray(mp['Mstar'])
        elif 'SubhaloMassType' in mp_fields:
            mstar_data = np.asarray(mp['SubhaloMassType'])[:, 4] * 1e10 / 0.6774
        else:
            raise KeyError("Could not find stellar-mass field ('Mstar' or 'SubhaloMassType') in merging_population.")
    else:
        # Fallback to merger arrays on the object.
        z_data = np.asarray(sim_obj.z_merging_pop)
        if hasattr(sim_obj, 'subhalo_ids_mergers'):
            sid_raw = np.asarray(sim_obj.subhalo_ids_mergers)
        else:
            raise AttributeError("sim_obj must provide 'subhalo_ids_mergers' when sim_obj.pop['merging_population'] is unavailable.")
        if hasattr(sim_obj, 'Mstar_merging_pop'):
            mstar_data = np.asarray(sim_obj.Mstar_merging_pop)
        else:
            raise AttributeError("sim_obj must provide 'Mstar_merging_pop' when sim_obj.pop['merging_population'] is unavailable.")

    if sid_raw.ndim == 1:
        sid_keys = [int(x) for x in sid_raw]
    elif sid_raw.ndim == 2:
        sid_keys = [tuple(int(v) for v in row) for row in sid_raw]
    else:
        sid_keys = [tuple(np.asarray(x).ravel().astype(int).tolist()) for x in sid_raw]

    key_to_indices = defaultdict(list)
    for i, (z, sid, ms) in enumerate(zip(z_data, sid_keys, mstar_data)):
        if np.isfinite(ms) and ms > 0:
            key = (float(z), sid, float(np.round(np.log10(ms), round_decimals)))
            key_to_indices[key].append(i)

    duplicate_groups = {k: idxs for k, idxs in key_to_indices.items() if len(idxs) > 1}
    duplicate_indices = sorted([idx for idxs in duplicate_groups.values() for idx in idxs])

    mask = np.zeros(n_mergers, dtype=bool)
    if len(duplicate_indices) > 0:
        mask[np.asarray(duplicate_indices, dtype=int)] = True

    sim_obj_filtered = copy.copy(sim_obj)

    # Filter any attribute aligned with merger rows.
    for attr_name, attr_value in vars(sim_obj).items():
        if isinstance(attr_value, np.ndarray) and attr_value.shape[0] == n_mergers:
            setattr(sim_obj_filtered, attr_name, attr_value[mask])
        elif isinstance(attr_value, list) and len(attr_value) == n_mergers:
            setattr(sim_obj_filtered, attr_name, [v for v, keep in zip(attr_value, mask) if keep])
        elif isinstance(attr_value, tuple) and len(attr_value) == n_mergers:
            setattr(sim_obj_filtered, attr_name, tuple(v for v, keep in zip(attr_value, mask) if keep))

    # Attach useful metadata for downstream inspection.
    sim_obj_filtered.multiple_merger_mask = mask
    sim_obj_filtered.multiple_merger_indices = np.asarray(duplicate_indices, dtype=int)
    sim_obj_filtered.multiple_merger_groups = duplicate_groups

    # Provide filtered raw merger table (in-memory dict) when available.
    if mp is not None:
        filtered_merging_population = {}
        for field in mp.keys():
            filtered_merging_population[field] = np.asarray(mp[field])[mask]
        sim_obj_filtered.filtered_merging_population = filtered_merging_population

    return sim_obj_filtered
...
def SBHAR_enhancement_calc_for_small_td(sim_name,sim_objs,merger_time_delay,td_threshold=0.5,type='massive',min_count=5):
    ...
    for z in unique_zs:
        ...
        if type == 'massive':
            merging_sBHAR_quantity = sim_objs[sim_name].sBHAR_merging_massive_pop[(sim_objs[sim_name].z_merging_pop==z) & small_td_mask]
            control_sBHAR_quantity = sim_objs[sim_name].sBHAR_control_massive_pop[(sim_objs[sim_name].z_merging_pop==z) & small_td_mask]

        elif type == 'luminous':
            merging_sBHAR_quantity = sim_objs[sim_name].sBHAR_merging_luminous_pop[(sim_objs[sim_name].z_merging_pop==z) & small_td_mask]
            control_sBHAR_quantity = sim_objs[sim_name].sBHAR_control_luminous_pop[(sim_objs[sim_name].z_merging_pop==z) & small_td_mask]

        else:
            merging_sBHAR_quantity = sim_objs[sim_name].sBHAR_merging_pop[(sim_objs[sim_name].z_merging_pop==z) & small_td_mask]
            control_sBHAR_quantity = sim_objs[sim_name].sBHAR_control_pop[(sim_objs[sim_name].z_merging_pop==z) & small_td_mask]

        sBHAR_enhancement = []
        for i in range(len(control_sBHAR_quantity)):
            if control_sBHAR_quantity[i]>0 and merging_sBHAR_quantity[i]>0:
                sBHAR_enhancement.append(np.log10(merging_sBHAR_quantity[i]/control_sBHAR_quantity[i]))
        
        avg_sBHAR_enhancement.append(np.mean(sBHAR_enhancement))
        std_sBHAR_enhancement.append(np.std(sBHAR_enhancement)/np.sqrt(len(sBHAR_enhancement)))
    
    avg_sBHAR_enhancement = np.array(avg_sBHAR_enhancement)
    std_sBHAR_enhancement = np.array(std_sBHAR_enhancement) 
    
    return filtered_unique_zs,avg_sBHAR_enhancement, std_sBHAR_enhancement
